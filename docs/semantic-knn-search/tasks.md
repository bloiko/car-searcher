...
| # | Task | Status | Reqs | PR |
|---|---|---|---|---|
| 1 | `CarSearchService`'s `must` clause becomes a `neural` query against `description_vector` (replacing `multi_match`), using the configured embedding model ID; `CarSearchRequest`'s stale javadoc updated | in_progress | R1.1, R1.2, R2.1, R2.2 |  |
